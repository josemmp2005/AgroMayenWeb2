  ...
  return (
    <header 
      className={`fixed top-0 left-0 right-0 z-50 transition-all duration-300 ${
        isScrolled ? 'py-3 glass shadow-md border-b border-brand-lime/20' : 'py-6 bg-transparent'
      }`}
    >
        ...
        <nav className="hidden md:flex gap-8">
          {navLinks.map((link) => (
            <a
              key={link.name}
              href={link.href}
              className={`font-medium hover:text-brand-lime transition-all relative after:content-[''] after:absolute after:w-0 after:h-0.5 after:bg-brand-lime after:left-0 after:-bottom-1 after:transition-all hover:after:w-full ${
      ...
      <div className={`fixed inset-0 bg-white z-40 transition-transform duration-300 md:hidden ${
        ...
        <div className="flex flex-col items-center justify-center h-full gap-8 bg-gradient-to-b from-white to-stone-100">
          <button 
